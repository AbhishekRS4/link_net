...
from link_net_model import LinkNet
from link_net_utils import init, read_config_file, get_tf_dataset

os.environ["TF_CPP_MIN_LOG_LEVEL"] = "3"
param_config_file_name = os.path.join(os.getcwd(), "link_net_config.json")
...
def compute_loss(groundtruth, prediction, axis=1, name="mean_cross_entropy"):
    ...
    mean_ce = tf.reduce_mean(
        tf.nn.sparse_softmax_cross_entropy_with_logits(labels=groundtruth, logits=prediction),
        name=name
    )
...
def batch_train(FLAGS):
    ...
    link_net.resnet34_encoder(features)
    link_net.link_net_decoder()
    logits = link_net.logits
    print("linknet model built")
