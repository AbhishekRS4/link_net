...
class LinkNet:
    ...
    def link_net_decoder(self):
        self.decoder1_out = self._get_decoder_block(self.stage5, 512, 256, [2, 2], name="decoder1_")
        self.decoder1_fuse = tf.add(self.decoder1_out, self.stage4, name="decoder1_fuse")
        self.decoder2_out = self._get_decoder_block(self.decoder1_fuse, 256, 128, [2, 2], name="decoder2_")
        self.decoder2_fuse = tf.add(self.decoder2_out, self.stage3, name="decoder2_fuse")
        self.decoder3_out = self._get_decoder_block(self.decoder2_fuse, 128, 64, [2, 2], name="decoder3_")
        self.decoder3_fuse = tf.add(self.decoder3_out, self.stage2, name="decoder3_fuse")
        self.decoder4_out = self._get_decoder_block(self.decoder3_fuse, 64, 64, [1, 1], name="decoder4_")

        self.decoder5_conv_tr = self._get_conv2d_transpose_layer(
            self.decoder4_out, 32, [3, 3], [2, 2], name="decoder5_conv_tr"
        )
        self.decoder5_bn_tr = self._get_batchnorm_layer(self.decoder5_conv_tr, name="decoder5_bn_tr")
        self.decoder5_relu_tr = self._get_relu_activation(self.decoder5_bn_tr, name="decoder5_relu_tr")

        self.decoder5_conv = self._get_conv2d_layer(
            self.decoder5_relu_tr, 32, [3, 3], [1, 1], name="decoder5_conv"
        )
        self.decoder5_bn = self._get_batchnorm_layer(self.decoder5_conv, name="decoder5_bn")
        self.decoder5_relu = self._get_relu_activation(self.decoder5_bn, name="decoder5_relu")
    ...
    def _get_decoder_block(self, input_layer, in_kernels, out_kernels, strides, name="decoder_"):
        x = self._get_conv2d_layer(input_layer, int(in_kernels // 4), [1, 1], [1, 1], name=name + "conv1")
        x = self._get_batchnorm_layer(x, name=name + "bn1")
        x = self._get_relu_activation(x, name=name + "relu1")

        x = self._get_conv2d_transpose_layer(x, int(in_kernels // 4), [3, 3], strides, name=name + "conv_tr")
        x = self._get_batchnorm_layer(x, name=name + "bn_tr")
        x = self._get_relu_activation(x, name=name + "relu_tr")

        x = self._get_conv2d_layer(x, out_kernels, [1, 1], [1, 1], name=name + "conv2")
    ...
    def _get_relu_activation(self, input_tensor, name="relu"):
        relu_layer = tf.nn.relu(input_tensor, name=name)
        return relu_layer
    ...
    def _conv_layer(self, input_layer, name, strides=[1, 1, 1, 1]):
        ...
        W_init_value = np.array(self._weights_h5[name][hierarchy_name]["kernel:0"], dtype=np.float32)
        W = tf.get_variable(
            name=name + "_kernel", shape=W_init_value.shape,
            initializer=tf.constant_initializer(W_init_value), dtype=tf.float32
        )
        x = tf.nn.conv2d(
            input_layer, filter=W, strides=strides, padding=self._padding,
